...
class TestTransposeForDisplay:
    ...
    def test_flip_vertical(
        ...
        assert (
            ttfttf == matrix
        ).all(), "flip twice, transpose 4 times should be identity"
        ...
        assert (
            tftftftf == matrix
        ).all(), "transpose and flip 4 times should be identity"

        tftf = tf_f(tf_f(matrix))
        assert (tftf != matrix).any(), "transpose and flip twice should not be identity"
